...
export(int,"Default","Control","Shift","Disabled") var running_input := 0
...
func fly(delta):
	flying_direction = 0
	if camera_input:
		flying_direction = clamp(Ap.head.rotation.x,-1.5,1.5) * (Input.get_action_strength("move_forward") - Input.get_action_strength("move_back"))
	...
	if minecraft_style and (Input.is_action_pressed("jump") or Input.is_key_pressed(KEY_SHIFT)):
...
func _ready() -> void:
	speed_rate = speed_rate if speed_rate > 0 else 1.0
	speed_limit = speed_limit if speed_limit > 0 else speed*2
	timer.one_shot = true
	...
	if enabled:
		if godot_style:
			if Input.is_key_pressed(KEY_E):
				...
				is_flying = true
			if Input.is_key_pressed(KEY_Q) and Ap.player.is_on_floor() and gravity_enabled:
